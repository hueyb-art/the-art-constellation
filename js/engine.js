...
let W,H,DPR;
function resize(){DPR=window.devicePixelRatio||1;W=innerWidth;H=innerHeight;canvas.width=W*DPR;canvas.height=H*DPR;canvas.style.width=W+"px";canvas.style.height=H+"px";ctx.setTransform(DPR,0,0,DPR,0,0);if(typeof tlMeasureChrome==="function")tlMeasureChrome();}
addEventListener("resize",resize);resize();

const radius=nd=>6+Math.sqrt(nd.deg)*3.0;
let activeEras=new Set();
let instrFilter=null;
var tlAxisY=74;   /* screen-y of the timeline year axis; kept below the toolbar. var (not let) so the startup resize() can set it before this line runs */
...
/* ========== TIMELINE VIEW — movements as dated clusters on a year axis ==========
...
let _tlKey="", timelineMoves=[], tlMaxLane=0, tlHoverMv=null, tlBloomMv=null;
const TL_BLOOM_K=1.3, TL_BLOOM_MAX=98;   /* hover fan-out: extra scale, capped radius */
/* Keep the year axis just below the top toolbar (which wraps taller on narrow
   screens), so the decade labels never hide behind it. */
...
function clampTL(x,y,z){
  ...
  /* vertical band = below the year axis, above the bottom corners */
...
function stepTimeline(){
  ...
  /* a movement chosen from the filter stays bloomed open, even without a hover */
  const forced=instrFilter?timelineMoves.find(m=>m.name===instrFilter):null;
...
/* Pan the timeline so a chosen movement's cluster sits centred in the band
   (used when a single movement is picked from the filter). */
function jumpToMovement(name){
  layoutTimeline(); tlMeasureChrome();
  const mv=timelineMoves.find(m=>m.name===name); if(!mv){frameTimeline();return;}
...
function loop(){
  ...
  if(!pointer.down){
    ...
    /* While an artist is selected, hold the sky still. The idle drift would
       otherwise carry their star out of frame within seconds of a search
       landing on it. Closing the card lets the constellation turn again. */
...
/* PINNING. Selecting an artist (search, or a click) pins the globe to their
   ...
   slides straight back out of frame — the layout never fully settles (alpha
   floors at 0.05) and, once centred, a star sits near the camera plane where
   the perspective factor is at its cap, so a pixel of drift in world space
   throws it right across the screen. Dragging hands control back to the user;
   deselecting releases the pin and the sky drifts again. */
var pinned=false;   /* var, not let: loop()/the drag handlers are defined above this line */
...
function renderPanel(nd){
  ...
  panelBody.innerHTML=`
    ...
    <div class="conns">${cs.map((c,i)=>G.noAudio
      ? `<div class="conn" data-id="${c.other.id}"><span class="rel">${esc(c.rel)}</span><span class="nm">${esc(c.other.name)}</span></div>${c.note?`<div class="connnote">${esc(c.note)}</div>`:""}`
      : `<div class="conn" data-id="${c.other.id}"><span class="rel">${c.rel}</span><span class="nm">${c.other.name}</span><span class="cx" data-i="${i}" title="Shared recordings">♪</span></div><div class="collab" id="cb${i}"></div>`).join("")}</div>`;
  ...
  if(!G.noAudio)panelBody.querySelectorAll(".cx").forEach(el=>{el.onclick=ev=>{ev.stopPropagation();const i=+el.dataset.i;toggleCollab(document.getElementById("cb"+i),nd,byId[cs[i].other.id],el);};});
}
...
function centerOn(nd){
  ...
  pinned=true;}
/* Re-aim at the pinned star. Called every frame from loop() so the star holds
   its place while the layout keeps breathing underneath it. */
...
{const _rr=document.getElementById("rrBtn");if(_rr)_rr.onclick=()=>openRooms();}
...
function playClip(nd){
  if(!clip)return;
  if(G&&G.noAudio)return;   /* visual-art build: no preview audio */
  clipFor=nd.id;
...
function setView(mode){
  ...
  /* the Tight/Spread slider only drives the globe force-sim — hide it elsewhere */
  ...
  else if(mode==="timeline"){deselect();frameTimeline();if(carry)select(carry,true);}
  else{NODES.forEach(nd=>{nd.vz+=(Math.random()-.5)*8;});tviewX=0;tviewY=0;setTimeout(fitView,1800);if(carry)select(carry,true);}
}
if(chordBtn)chordBtn.onclick=()=>setView((G&&G.noAudio)?(viewMode==="timeline"?"globe":"timeline"):(viewMode==="chord"?"globe":"chord"));
...
function loadGenre(key){
  ...
  NODES.forEach(nd=>{
    ...
    nd.instr=nd.movement||instrumentOf(nd.role);   /* art: filter by movement; music: by instrument */
    nd.roleTag=(nd.role||"").split("·")[0].trim();   /* short tag under the name — movement (art) / instrument (music) */
...
instrEl.onchange=()=>{instrFilter=instrEl.value||null;alpha=Math.max(alpha,0.8);userFramed=true;
  if(viewMode==="timeline"){deselect();if(instrFilter)jumpToMovement(instrFilter);else frameTimeline();}

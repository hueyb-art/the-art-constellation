...
- **Zoom is anchored to the pointer** (`tlZoomAt`). Screen x is `W*0.04 + worldX*zoom + viewX`, so changing zoom alone expands everything away from the origin and the years under the cursor slide away — pinching to zoom into a cluster visibly threw the timeline back to earlier decades. The world point under the cursor (or the midpoint between two fingers) is now held fixed. A horizontal wheel/trackpad swipe pans through time rather than zooming.
